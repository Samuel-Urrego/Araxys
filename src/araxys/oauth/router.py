...
class _StateStore:
    """Thread-safe in-memory store for OAuth2 state ↔ PKCE verifier."""
    ...
    def _cleanup(self) -> None:
        now = time.time()
        expired = [s for s, (_, e) in self._store.items() if now > e]
        for s in expired:
...
def create_oauth_router(
    ...
    *,
    ...
    async def callback(
        ...
        redirect_uri = str(router.prefix or "") + f"/callback/{provider}"
