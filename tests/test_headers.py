...
@pytest.fixture
async def headers_client(headers_app: FastAPI) -> AsyncClient:
...
class TestSecureHeaders:
    async def test_hsts_header(self, headers_client: AsyncClient):
    ...
    async def test_content_type_nosniff(self, headers_client: AsyncClient):
        response = await headers_client.get("/test")
        assert response.headers.get("x-content-type-options") == "nosniff"

    async def test_frame_options(self, headers_client: AsyncClient):
        response = await headers_client.get("/test")
        assert response.headers.get("x-frame-options") == "DENY"

    async def test_xss_protection_disabled(self, headers_client: AsyncClient):
        response = await headers_client.get("/test")
        assert response.headers.get("x-xss-protection") == "0"

    async def test_referrer_policy(self, headers_client: AsyncClient):
        response = await headers_client.get("/test")
        assert (
    ...
    async def test_csp_not_set_by_default(self, headers_client: AsyncClient):
        response = await headers_client.get("/test")
